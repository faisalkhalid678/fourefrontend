...
export default function TravelportTicket({ bookingData, query }) {
    ...
    useEffect(() => {
        ...
        const TravelportTicketAPI = '/issue-ticket';

        async function TravelportGenerateTicket() {
            if (SaleLoading && PayResponse.status === '200') {
                Axios.post(TravelportTicketAPI, TicketData)
                    .then((response) => {
    ...
    const dueDate = (bookingData.data.pricing.length > 0) ? date_convert(bookingData.data.pricing[0].TrueLastDateToTicket) + ' ' + utc_convert(bookingData.data.pricing[0].TrueLastDateToTicket) + ' ' + TimeZone(bookingData.data.pricing[0].TrueLastDateToTicket) : date_convert(new Date()) + ' ' + utc_convert(new Date().toLocaleString()) + ' UTC +05:00';
    ...
    // PDF Print Function
    function print() {
        savePdf(<Document bookingData={bookingData} creationDate={creationDate} totalStops={totalStops} totalFlightTime={totalFlightTime} 
            firstSegment={firstSegment} lastSegment={lastSegment} round={round} QueryCity={QueryCity} InvoiceData={InvoiceData} />, 
                                                        ...
                                                        <div className="PAX-info row">
                                                            ...
                                                            <div className="right">
                                                                ...
                                                                <div className="PAX-inner-row">
                                                                    ...
                                                                    <p>
                                                                        {bookingData.data.passenger_detail[0].exp_day + ' ' + bookingData.data.passenger_detail[0].exp_month + ' ' + bookingData.data.passenger_detail[0].exp_year }
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
