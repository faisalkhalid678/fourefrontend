...
const MyDocument = ({ bookingData, totalStops, totalFlightTime, firstSegment, lastSegment, round, TotalPrice, ticketing }) => (
    <Document>
                ...
                <View style={styles.PAXInfo}>
                    {
                        (bookingData.data.passenger_detail[0].nationality) ?
                            <>
                                <Text style={styles.left}>Passenger: {bookingData.data.passenger_detail[0].firstName + " " + bookingData.data.passenger_detail[0].lastName}</Text>
                                <Text style={styles.right}>Nationality: {toUpper(bookingData.data.passenger_detail[0].nationality)}</Text>
                                <Text style={styles.left}>PNR Creation Date: {date_convert(bookingData.data.CreateDateTime)}</Text>
                        ...
                        :
                            <>
                                <Text style={styles.left}>Passenger: {bookingData.data.passenger_detail[0].firstName + " " + bookingData.data.passenger_detail[0].lastName}</Text>
                                <Text style={styles.right}>PNR Creation Date: {date_convert(bookingData.data.CreateDateTime)}</Text>
                                <Text style={styles.left}>Booking Reference (PNR): {bookingData.data.galilo_pnr}</Text>
                                <Text style={styles.right}>Fare: {TotalPrice}</Text>
                                <Text style={styles.left}>Issuing Agent: Bukhari Travel Services</Text>
                                <Text style={styles.right}>IATA Number: 27303054</Text>
                            </>
                    }
                        ...
                        return (
                            <View style={styles.flightsParent} key={Math.random()}>
                                ...
                                <View style={styles.tableParent}>
                                            ...
                                            bookingData.data.passenger_detail.map((pax, index) => {
                                                return (
                                                    <View style={styles.tableRow} key={Math.random()}>
                                                        <View style={styles.tableCol}>
                                                            <Text style={styles.tableCell}>{pax.lastName + ', ' + pax.firstName}</Text>
                                                        </View>
                                                        <View style={styles.tableCol}>
                                                            {
                                                                (bookingData.data.ticket_numbers && bookingData.data.ticket_numbers.length > 0) ?
                                                                    <Text style={styles.tableCell}>{bookingData.data.ticket_numbers[index].ticket_number}</Text>
                                                                    :
                                    ...
                                    <Text style={styles.service}>Class of Service: {segment.CabinClass}</Text>
