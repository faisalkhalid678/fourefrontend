...
const MyDocument = ({ bookingData, ticketing, totalStops, lastSegment, firstSegment, round }) => (
    <Document>
        <Page size="A4" style={{ paddingTop: 25, paddingBottom: 25, paddingLeft: 25, paddingRight: 25, }}>
            ...
            <View style={styles.head}>
                ...
                <View style={styles.PAXInfo}>
                    ...
                    <Text style={styles.right}>Nationality: {(bookingData.data.passenger_detail[0].cnic) ? 'PK' : toUpper(bookingData.data.passenger_detail[0].nationality)}</Text>
                    ...
                    <Text style={styles.left}>Booking Reference (PNR): {bookingData.data.pnr}</Text>
                    {
                        ...
                        :
                            (bookingData.data.passenger_detail[0].cnic != null && bookingData.data.passenger_detail[0].cnic != "_____-_______-_") &&
                                <Text style={styles.right}>CNIC: {bookingData.data.passenger_detail[0].cnic}</Text>
                    }
                    {
                        (bookingData.data.passenger_detail[0].passport_number != null) &&
                            <Text style={styles.left}>Passport Expiry: {bookingData.data.passenger_detail[0].exp_day + " " + bookingData.data.passenger_detail[0].exp_month + " " + bookingData.data.passenger_detail[0].exp_year}</Text>
                    }
                </View>
                ...
                <View>
                    <View style={styles.flightDetailsParent}>
                        ...
                        <View style={styles.FlightInfo}>
                            <Text style={styles.ParentStops}>{(totalStops === 0) ? 'Direct Flight' : (totalStops === 1) ? totalStops + ' Stop' : totalStops + ' Stops'}</Text>
                            <Image style={styles.ParentArrow} src={BlueArrow} />
                            <Text>Total Flight Time:{' ' + diff_minutes(lastSegment.ArrivalTime, firstSegment.DepartureTime)}</Text>
                                                        ...
                                                        <View style={styles.tableCol}>
                                                            {
                                                                (bookingData.data.tickets) ?
                                                                    <Text style={styles.tableCell}>{bookingData.data.tickets[index].ticket_number}</Text>
                                                                :
